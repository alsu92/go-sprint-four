...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((float64(runningCaloriesMeanSpeedMultiplier) * meanSpeed * runningCaloriesMeanSpeedShift) * weight / float64(mInKm) * duration * float64(minInH))
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	meanSpeed := math.Pow(meanSpeedInMsec, 2) / (height / cmInM) // (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах)
	return ((walkingCaloriesWeightMultiplier*weight + meanSpeed*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH))
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	return (meanSpeed + swimmingCaloriesMeanSpeedShift) * float64(swimmingCaloriesWeightMultiplier) * weight * duration
}
